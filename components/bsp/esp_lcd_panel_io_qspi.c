...
static const char * TAG = "lcd_panel.io.qspi";

static esp_err_t panel_io_qspi_rx_param(esp_lcd_panel_io_t * io, int lcd_cmd, void * param, size_t param_size);
static esp_err_t panel_io_qspi_tx_param(esp_lcd_panel_io_t * io, int lcd_cmd, const void * param, size_t param_size);
static esp_err_t panel_io_qspi_tx_color(esp_lcd_panel_io_t * io, int lcd_cmd, const void * color, size_t color_size);
static esp_err_t panel_io_qspi_del(esp_lcd_panel_io_t * io);
static void lcd_qspi_pre_trans_cb(spi_transaction_t * trans);
static void lcd_qspi_post_trans_color_cb(spi_transaction_t * trans);
static esp_err_t panel_io_qspi_register_event_callbacks(esp_lcd_panel_io_handle_t io,
...
lcd_qspi_trans_descriptor_t trans_pool[];			// Transaction pool
} esp_lcd_panel_io_qspi_t;


esp_err_t esp_lcd_new_panel_io_qspi(esp_lcd_spi_bus_handle_t bus, const esp_lcd_panel_io_spi_config_t * io_config,
	 esp_lcd_panel_io_handle_t * ret_io)
{
	...
	esp_lcd_panel_io_qspi_t * spi_panel_io = NULL;

	ESP_GOTO_ON_FALSE(bus && io_config && ret_io, ESP_ERR_INVALID_ARG, err, TAG, "invalid argument");
	spi_panel_io		= calloc(1,
		 sizeof(esp_lcd_panel_io_qspi_t) +sizeof(lcd_qspi_trans_descriptor_t) *io_config->trans_queue_depth);
	ESP_GOTO_ON_FALSE(spi_panel_io, ESP_ERR_NO_MEM, err, TAG, "no mem for spi panel io");

	spi_device_interface_config_t devcfg =
	{
		.flags				=
			 SPI_DEVICE_HALFDUPLEX | (io_config->flags.lsb_first ? SPI_DEVICE_TXBIT_LSBFIRST: 0) | (io_config->flags.cs_high_active ? SPI_DEVICE_POSITIVE_CS: 0),
		...
		.pre_cb 			= lcd_qspi_pre_trans_cb, // pre-transaction callback, mainly control DC gpio level
		.post_cb			= lcd_qspi_post_trans_color_cb, // post-transaction, where we invoke user registered "on_color_trans_done()"
	};
	...
	spi_panel_io->base.rx_param = panel_io_qspi_rx_param;
	spi_panel_io->base.tx_param = panel_io_qspi_tx_param;
	spi_panel_io->base.tx_color = panel_io_qspi_tx_color;
	spi_panel_io->base.del = panel_io_qspi_del;
	spi_panel_io->base.register_event_callbacks = panel_io_qspi_register_event_callbacks;
...
static esp_err_t panel_io_qspi_del(esp_lcd_panel_io_t * io)
{
	...
	esp_lcd_panel_io_qspi_t * spi_panel_io = __containerof(io, esp_lcd_panel_io_qspi_t, base);
...
static void spi_lcd_prepare_cmd_buffer(esp_lcd_panel_io_qspi_t * panel_io, const void * cmd)
{
	uint8_t *		from = (uint8_t *)
...
static void spi_lcd_prepare_param_buffer(esp_lcd_panel_io_qspi_t * panel_io, const void * param, size_t param_size)
{
	uint8_t *		from = (uint8_t *)
...
static esp_err_t panel_io_qspi_tx_param(esp_lcd_panel_io_t * io, int lcd_cmd, const void * param, size_t param_size)
{
	esp_err_t		ret = ESP_OK;
	spi_transaction_t * spi_trans = NULL;
	lcd_qspi_trans_descriptor_t * lcd_trans = NULL;
	esp_lcd_panel_io_qspi_t * spi_panel_io = __containerof(io, esp_lcd_panel_io_qspi_t, base);
	...
	memset(lcd_trans, 0, sizeof(lcd_qspi_trans_descriptor_t));
...
static esp_err_t panel_io_qspi_tx_color(esp_lcd_panel_io_t * io, int lcd_cmd, const void * color, size_t color_size)
{
	...
	do 
	{
		...
		else 
		{
			...
			ret 				= spi_device_get_trans_result(spi_panel_io->spi_dev, &spi_trans, portMAX_DELAY);
			ESP_GOTO_ON_ERROR(ret, err, TAG, "recycle spi transactions failed");
			lcd_trans			= __containerof(spi_trans, lcd_qspi_trans_descriptor_t, base);
			spi_panel_io->num_trans_inflight--;
		}
	...
	// continue while we have remaining data to transmit
...
static void lcd_qspi_pre_trans_cb(spi_transaction_t * trans)
{
	esp_lcd_panel_io_qspi_t * spi_panel_io = trans->user;
	lcd_qspi_trans_descriptor_t * lcd_trans = __containerof(trans, lcd_qspi_trans_descriptor_t, base);
