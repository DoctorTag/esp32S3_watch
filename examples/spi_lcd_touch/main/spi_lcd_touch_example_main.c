...
#if CONFIG_EXAMPLE_LCD_TOUCH_CONTROLLER_CST816
...
static void example_lvgl_touch_cb(lv_indev_drv_t * drv, lv_indev_data_t * data)
{
    ...
    if (touchpad_pressed && touchpad_cnt > 0) {
		...
		ESP_LOGI(TAG, "Touch x=%x,y=%x ", touchpad_x[0], touchpad_y[0]);
...
void app_main(void)
{
    ...
    spi_bus_config_t buscfg = {
        .sclk_io_num = EXAMPLE_PIN_NUM_SCLK,
        .mosi_io_num = EXAMPLE_PIN_NUM_D0,
        .miso_io_num = EXAMPLE_PIN_NUM_D1,
        .quadwp_io_num = EXAMPLE_PIN_NUM_D2,
        .quadhd_io_num = EXAMPLE_PIN_NUM_D3,
        .max_transfer_sz = EXAMPLE_LCD_H_RES * 80 * sizeof(uint16_t),
    };
    ESP_ERROR_CHECK(spi_bus_initialize(LCD_HOST, &buscfg, SPI_DMA_CH_AUTO));
...
#endif // CONFIG_EXAMPLE_LCD_TOUCH_CONTROLLER_CST816
